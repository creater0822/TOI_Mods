...
{
    public class UIDesignatePurpose : MonoBehaviour
    {
        ...
        public UIDesignatePurpose(IntPtr ptr)
            : base(ptr)
        {
        }
        ...
        private void Awake()
        {
            ...
            leftTitle.text = "Select features below";
        ...
        public void InitData(UIDaguiToolItem toolItem, int index)
        {
            ...
            {
                if (confSchoolSloganItem.type != 1 || confSchoolSloganItem.effect == "0" || string.IsNullOrWhiteSpace(GameTool.LS(confSchoolSloganItem.desc)) || string.IsNullOrWhiteSpace(GameTool.LS(confSchoolSloganItem.slogan)))
                {
                    continue;
                }
                ...
                obj.AddComponent<Button>().onClick.AddListener((Action)delegate
                {
                    ...
                    {
                        UITipItem.AddTip("Choose up to 2 features!", 0f);
                    }
                    else
                    {
                        if (this.selectItem.Contains(selectItem))
                        {
                            UITipItem.AddTip("Choose from different features at most!", 0f);
                        }
        ...
        public void OnBtnOk()
        {
            ...
            {
                UITipItem.AddTip("Choose at least 2 features!", 0f);
                return;
            }
